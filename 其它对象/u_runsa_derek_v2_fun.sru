$PBExportHeader$u_runsa_derek_v2_fun.sru
$PBExportComments$derek专用函数
forward
global type u_runsa_derek_v2_fun from nonvisualobject
end type
type shfileopstruct from structure within u_runsa_derek_v2_fun
end type
type md5_ctx from structure within u_runsa_derek_v2_fun
end type
type shellexecuteinfo from structure within u_runsa_derek_v2_fun
end type
type ole_vbscript from oleobject within u_runsa_derek_v2_fun
end type
type ids_temp from datastore within u_runsa_derek_v2_fun
end type
end forward
...
global u_runsa_derek_v2_fun u_runsa_derek_v2_fun
...
Function BOOLEAN GetExitCodeProcess( LONG hProcess,REF ULONG lpExitCode ) LIBRARY "KERNEL32.DLL" alias for "GetExitCodeProcess;ansi"
...
FUNCTION boolean TerminateProcess(ulong hProcess,ulong uExitCode) LIBRARY "kernel32.dll" ALIAS FOR "TerminateProcess;ansi"
...
public subroutine chartobyte (ref character ac_char[], ref byte lb_byte[])
public subroutine hextobyte (ref character lc_char[], ref byte lb_byte[])
public subroutine hextoasc (ref character lc_char[2], ref byte lb_byte)
...
public function integer uf_urldata (ref string ls_urldata, encoding lencoding, ref string ls_names[], ref string ls_values[])
public function string urldecode (readonly character as_text[], readonly encoding ae_encode)
...
public function string uf_run (string ls_runcmd, string ls_param)
end prototypes
...
l_hwd=fileopen(as_filepath,TextMode!,read!,lockwrite!)
if l_hwd=-1 then 
	ls_errtext='文件:'+as_filepath+'打开失败!'
	goto e
end if

l_length=filereadex(l_hwd,as_text)
...
if al_start<=1 then
	...
	l_length=filereadex(l_hwd,ab_text,al_start - 1)
	if l_length=-100 then l_length=0	//无内容
	if l_length>=0 then 
	else
...
if al_len<=0 then 
	l_length=filereadex(l_hwd,ab_text)
	if l_length=-100 then l_length=0	//无内容
	if l_length>=0 then 
	else
		messagebox('','文件:'+as_filepath+'读取失败!')
	...
	l_length=filereadex(l_hwd,ab_text,al_len)
	if l_length=-100 then l_length=0	//无内容
	if l_length>=0 then 
	else
		messagebox('','文件:'+as_filepath+'读取失败!')
...
fileop.pFrom =string( blob(Source,encodingansi!))
fileop.pTo = string(blob(Dest,encodingansi!))
...
l_length=filereadex(l_hwd,lb_header)
...
public subroutine chartobyte (ref character ac_char[], ref byte lb_byte[]);//char[] -> byte[]
//256->0
...
public subroutine hextobyte (ref character lc_char[], ref byte lb_byte[]);//将16进制串每两位转换->byte[]
...
public subroutine hextoasc (ref character lc_char[2], ref byte lb_byte);//两位16进制转换成10进制byte
...
public function string hexdecode (ref character ac_text[], readonly encoding ae_encode);//16进制字符串转换成指定编码的字符
//C1AACDA8 -> 联通
...
//chartobyte(lc_new[],lb_byte[])
lc_new[]=string(blob(lb_byte[]),ae_encode)
return lc_new[]
...
GetTextExtentPoint32(l_dc,ls_text, lena(ls_text) , lpSize)
...
public function integer uf_base64 (ref blob lb_content, ref string ls_result);byte lbyte_array[],lbyte_result[]
...
ls_command="regsvr32.exe";
if as_function='unreg' then 
	ls_command+=' /u /s "'+as_dllpath+'" ';
else
	ls_command+=' /s "'+as_dllpath+'" ';
end if

as_info=uf_run(ls_command,'verb=runas;')
if as_info<>'' then goto e //注册失败
return 1
...
public function integer uf_urldata (ref string ls_urldata, encoding lencoding, ref string ls_names[], ref string ls_values[]);//格式:arg1=value1{&arg2=value2{&...}}
string ls_args
int li_pos1,li_pos2
int l_count
char lc_temp[]
...
public function string urldecode (readonly character as_text[], readonly encoding ae_encode);//对中文字符进行url 解码
//参数1:要解码的字符串
//参数2:要以哪种字符集解码
...
/*args*/
string ls_directory,ls_verb
long l_parenthandle,l_waitms=65535 /*默认等待到结束*/
long l_show=1 /*默认显示*/
string ls_exefile_path,ls_cmdargs

string ls_key[]={'directory','parenthandle','show','verb','waitms'} 

/*需要从开头按顺序传入*/
if left(ls_param,len(ls_key[1]))=ls_key[1] then ls_directory=uf_cutvalue(ls_param,ls_key[1]+'=',';')
if left(ls_param,len(ls_key[2]))=ls_key[2] then l_parenthandle=long(uf_cutvalue(ls_param,ls_key[2]+'=',';'))
if left(ls_param,len(ls_key[3]))=ls_key[3] then l_show=integer(uf_cutvalue(ls_param,ls_key[3]+'=',';'))
if left(ls_param,len(ls_key[4]))=ls_key[4] then ls_verb=uf_cutvalue(ls_param,ls_key[4]+'=',';')
if left(ls_param,len(ls_key[5]))=ls_key[5] then 
	l_waitms=long(uf_cutvalue(ls_param,ls_key[5]+'=',';'))
end if

if left(ls_runcmd,1)='"' then 
	l_pos=pos(ls_runcmd,'"',2)
	ls_exefile_path=mid(ls_runcmd,2,l_pos - 2)
	ls_cmdargs=mid(ls_runcmd,l_pos + 1)
else
	if fileexists(ls_runcmd) then //优先检查是否存在
		ls_exefile_path=ls_runcmd
		ls_cmdargs=''
	else //拆分路径
		l_pos=pos(ls_runcmd,' ')
		if l_pos=0 then 
			...
			ls_exefile_path=mid(ls_runcmd,1,l_pos - 1)
			ls_cmdargs=mid(ls_runcmd,l_pos + 1)
		end if
	end if
end if
if ls_directory='' then //工作目录
	replaceex(ls_exefile_path,'/','\')
	l_pos=lastpos(ls_exefile_path,'\')
	if l_pos=0 then 
		ls_directory=''
	else
		ls_directory=left(ls_exefile_path,l_pos)
	end if
end if
...
sei.lpFile=ls_exefile_path
sei.nShow=l_show;
sei.lpDirectory=ls_Directory
sei.lpParameters=ls_cmdargs+' '+ls_param

li_ret=ShellExecuteEx(sei)
if li_ret<1 then 
	ls_retvalue='ShellExecuteEx 失败:'+string(li_ret)
	if fileexists(ls_exefile_path)=false then 
		ls_retvalue='指定路径不存在:'+ls_exefile_path
	end if
	goto e
end if

if l_waitms>0 then //等待运行
	//等待结束
	WaitForSingleObject(sei.hProcess,l_waitms) //等待的毫秒数,65535为一直等到结束
	
	ULong lpExitCode
	boolean lb_ret
	lb_ret = GetExitCodeProcess ( sei.hProcess, ref lpExitCode ) //lpExitCode 259=超时,1失败,0成功
	choose case lpExitCode
		case 0 //程序正常退出,触发了app.event close/cmd无错误exit/bat运行结束或exit/
			
		case 1 
			ls_retvalue='进程已退出'  //命令未成功/进程被kill/explorer仅一个进程
			goto e
		case 255
			ls_retvalue='批处理被中断'  //bat-->ctrl+C中断
			goto e
		case 259 
			lb_ret=TerminateProcess(sei.hProcess,0)
	//		uf_run("taskkill.exe /PID "+string(sei.hProcess)+" /T",'show=0;')
	//		uf_run("taskkill.exe /PID "+string(sei.hProcess)+" /T /F",'show=0;')
			//taskkill /PID %1 /T /F
			ls_retvalue='无响应'
			goto e
		case 9009
			ls_retvalue='cmd期间存在错误'  //cmd执行了错误命令再exit/cmd调用cmd则会重新记录
			goto e
		case else
			ls_retvalue='操作被中断:'+string(lpExitCode,'') //halt/bat或cmd窗口被close/
			goto e
	end choose
end if
...
on u_runsa_derek_v2_fun.create
call super::create
...
on u_runsa_derek_v2_fun.destroy
...
type ole_vbscript from oleobject within u_runsa_derek_v2_fun descriptor "pb_nvo" = "true" 
end type
...
type ids_temp from datastore within u_runsa_derek_v2_fun descriptor "pb_nvo" = "true" 
end type
