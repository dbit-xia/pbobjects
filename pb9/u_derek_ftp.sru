...
il_ftpconn = InternetConnect(il_netconn,as_servername,ai_port,as_username,as_password,INTERNET_SERVICE_FTP,0,0) 
If il_ftpconn = 0 Then
	uf_getlastinfo(l_code,ls_info)
...
IF lb_ok = False Then
	...
	if strInfo='' then 
		...
		elseif ls_lastchar='/' or ls_lastchar='\' then
			strInfo='本地路径"'+as_newfile+'"必须指定文件名称!'
		else
			strInfo='本地路径"'+as_newfile+'"不存在!'
		end if
	end if
	Return 'Get失败 '+string(strInfo)
End If
...
//	return 'Put失败 源文件'+as_localfile+'不存在!'
//end if
...
IF lb_ok = False Then
	...
	if strInfo='' then 
		...
		if as_remotefile='' then 
			strInfo='远程路径不能为空!'
		elseif ls_lastchar='/' or ls_lastchar='\' then
			strInfo='本地路径"'+as_localfile+'"必须指定文件名称!'
		else
			strInfo='本地路径"'+as_localfile+'"不存在!'
		end if
	end if
	Return 'Put失败 '+string(strInfo)
End If
...
do
	...
	li_ret=FileWrite(li_fileno,BlobMid(lb_buffer,1,l_realsize))
	if li_ret=-1 then 
