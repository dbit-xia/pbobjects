...
type ids_empty from datastore within u_derek_v2_dd
end type
end forward
...
string is_listname[]
string is_listsql[]
string is_nodename[],is_fieldname[]
string is_keystring[]
datastore ids_list[]
boolean ib_dsbuild[]
int i_listcount,i_nodecount[]
datastore ids_nodes
datastore ids_fields
int i_keycount[]
datastore ids_wherekeys
...
public function integer uf_setitem (powerobject dd, long al_row, string as_columname, any a_value, integer ai_option)
...
public function integer uf_setcell (powerobject dd, long row, string column)
...
public function string uf_import (powerobject dd, ref string ls_inputcols, long l_startrow, string ls_startcolumn, string ls_text)
...
public subroutine uf_sql_explain (character ls_sql[], ref string ls_newsql, ref string ls_aliasname[], ref string ls_expression[])
...
ls_dwsyntax='release 9;~r~n table(' 
int li_count
long i
...
if l_lastpos>0 then 
	...
	ls_prop=dd.dynamic describe("Evaluate('"+ls_prop+"',"+string(row)+")")
end if
...
public function integer uf_setitem (powerobject dd, long al_row, string as_columname, any a_value, integer ai_option);//设置any多种类型的值
...
ls_coltype=dd.dynamic describe(as_columname+".coltype")
...
return dd.dynamic setitem(al_row,as_columname,la_value)
...
public function integer uf_setcell (powerobject dd, long row, string column);if string(column,'')='' or row<=0 then goto e
if dd.dynamic describe(column+".type")<>'column' then goto e

if dd.dynamic getrow()<>row or dd.dynamic getcolumnname()<>column then 
	if dd.dynamic accepttext()=-1 then goto e
	if  dd.dynamic getrow()<>row then 
		if dd.dynamic setrow(row)=-1 then goto e
		if dd.dynamic getrow()<>row then goto e
	end if
	if dd.dynamic getcolumnname()<>column then
		if dd.dynamic setcolumn(column)=-1 then goto e
		if dd.dynamic getcolumnname()<>column then goto e
	end if
end if
...
string ls_key[]={'column','dataobject','settrans'}
string ls_tablevalue,ls_dataobject,ls_settrans
...
if left(as_sql,len(ls_key[1]))=ls_key[1] then 
	ls_tablevalue=uf_cutvalue(as_sql,ls_key[1]+'=',';')
end if
if left(as_sql,len(ls_key[2]))=ls_key[2] then 
	ls_dataobject=uf_cutvalue(as_sql,ls_key[2]+'=',';')
end if
//if left(as_sql,len(ls_key[3]))=ls_key[3] then 
//	ls_settrans=uf_cutvalue(as_sql,ls_key[3]+'=',';')
//end if


if ls_tablevalue<>'' then 
	l_syntax="release 9;table("
...
//		end if
	...
	for i=1 to l_colcount
		l_syntax+="column=(type="+ls_type[i]+" name="+ls_name[i]+" dbname=~""+ls_name[i]+"~" update="+ls_update[i]+" updatewhereclause="+ls_updatewhere[i]+" initial=~""+ls_initial[i]+"~") ~r~n"
	next
...
if li_up<=0 then //全部列
	...
	for i=1 to li_up
		ls_xml+="<"+trim(ls_eles[i])+">"+ls_column[i]+"</"+trim(ls_eles[i])+">"
	next
end if
	...
	case 0	//返回object:columns+computes
		...
		for i=1 to li_count
			if dd.dynamic describe(ls_objects[i]+".band")='detail' and (dd.dynamic describe(ls_objects[i]+".type")='column' or dd.dynamic describe(ls_objects[i]+".type")='compute' ) then
				...
				lobj.data[j,1]=dec(dd.dynamic describe(ls_objects[i]+".x"))
				lobj.data[j,2]=dd.dynamic describe(ls_objects[i]+".name")
...
//		next
	...
	case 4 //列排序
		...
		for i=1 to li_count
			lobj.data[i,1]=dec(dd.dynamic describe(ls_column[i]+".x"))
			lobj.data[i,2]=dd.dynamic describe(ls_column[i]+".name")
...
is_nodename[1]='产品信息' 
is_nodename[2]='产品货号,产品名称=,条码信息,产品价格,尺寸类型,1:row,'
is_nodename[3]='type=,row'
is_nodename[4]='type=,颜色代号=,颜色信息,尺寸代号,条码,'
is_nodename[5]='颜色名称,'
is_nodename[6]='颜色代号,颜色信息'

is_fieldname[1]=','
is_fieldname[2]="colthno,~"co~~lthname~",,sprice,sizekind,,"
is_fieldname[3]="~"list~",,"
is_fieldname[4]="~"list~",color,,size,words,"
is_fieldname[5]='color_name'
is_fieldname[6]='color,,'

is_listsql[1]=""
is_listsql[2]="select colthno,colthname,sprice,sizekind from coloth_t where colthno like '%ANTA%' order by colthno"
is_listsql[3]=""
is_listsql[4]="select colthno,color=substring(color_nos,1,3),size=substring(color_nos,5,10),words from coloth_easy where colthno like '%ANTA%' order by colthno,color,size,words"
is_listsql[5]="select color=color_no,color_name from gg_color order by color_no "
is_listsql[6]="select distinct colthno,color=substring(color_nos,1,3) from coloth_easy where colthno like '%ANTA%' order by colthno,color"
...
choose case attrib
	case 'update','updatewhereclause','key','edit.focusrectangle'
		if as_values='1' then 
			as_value='yes'
		...
		for i=1 to l_cols
			...
			ls_string+=as_columns[i]+'.'+attrib+'='+as_value+' '
		next
	case 'tabsequence'
		for i=1 to l_cols
			if as_columns[i]='' then continue;
...
public function string uf_xmlout (ref string ls_text);string ls_errtext

if uf_xmlout_create(ls_errtext)=-1 then goto e

uf_xmlout_append(ls_text,is_listname[1],ids_empty,0)
...
for i=1 to i_listcount
	...
	if upperbound(ids_list[])>=i then //防止数组越界
		if isvalid(ids_list[i]) then continue;
	end if
	ids_list[i]=create datastore
	ib_dsbuild[i]=true
	ls_errtext=getquery(ids_list[i],is_listsql[i])
	if ls_errtext<>'' then goto e
next
...
if ids_nodes.create(ls_dwsyntax,ls_errtext)=-1 then 
	ls_errtext='Create : '+ls_errtext
	goto e
end if

ids_fields=create datastore
if ids_fields.create(ls_dwsyntax,ls_errtext)=-1 then 
	ls_errtext='Create : '+ls_errtext
	goto e
end if

ids_wherekeys=create datastore
if ids_wherekeys.create(ls_dwsyntax,ls_errtext)=-1 then 
	ls_errtext='Create : '+ls_errtext
	goto e
end if
...
for i=1 to i_listcount
	...
	//对应字段
	li_upbound=getarray(is_fieldname[i],ls_array[],',')
...
public function string uf_import (powerobject dd, ref string ls_inputcols, long l_startrow, string ls_startcolumn, string ls_text);//作用:分析数据窗口选中的矩形 ,矩形覆盖导入
//powerobject dd:datawindow/datastore
//string as_text:要粘贴的字符
//返回''为成功,否则为失败信息
...
//默认为当前行
if l_startrow<=0 then l_startrow=dd.dynamic getrow() 
if l_startrow>dd.dynamic rowcount() then goto e

//默认为当前列
if ls_startcolumn='' then 
	ls_startcolumn=dd.dynamic getcolumnname()
else
	ls_colname=dd.dynamic describe(ls_startcolumn+".name")
end if
...
for i=1 to l_textrowcount
	...
	l_row++
	if l_row>dd.dynamic rowcount() then goto e
	...
	for m=1 to li_minhcount
		...
		ls_celltext=string(ids_temp.object.data[i,m],'')
		
		li_colno=li_startcolno + m - 1
		
		if uf_getmodifiable(dd,l_row,ls_cols[li_colno])=false then continue;	//忽略不可修改的单元格
		
		li_ret=uf_settext(dd,l_row,ls_cols[li_colno],ls_celltext)
...
ls_syntax1=getdwosyntax(ls_dwsyntax,ls_dwotype[1],'id='+string(li_colid)+' ')
...
ls_expression="~~~""+ls_quote+"~~~" + lookupdisplay("+ls_columns[1]+") + ~~~""+ls_quote+"~~~" "
for i=2 to li_column_count
	ls_expression+=" + ~~~""+ls_separator+"~~~" + ~~~""+ls_quote+"~~~" + lookupdisplay("+ls_columns[i]+") + ~~~""+ls_quote+"~~~" "
next
...
if isvalid(lds_main)=false then //列表封装
	lds_main=ldw_parent
...
for l_row=l_startrow to l_endrow
	...
	for li_index=1 to i_nodecount[l_listindex]
		...
		if string(ls_fieldnames[li_index],'')='' then //如果是列表
			uf_xmlout_append(ls_text,ls_nodenames[li_index],lds_main,l_row)
			continue;
		end if
...
for l_row=1 to l_rowcount
	ls_ret=''
	uf_xmlout_append(ls_ret,is_listname[1],ids_list[li_mainlistindex],l_row)
...
public subroutine uf_sql_explain (character ls_sql[], ref string ls_newsql, ref string ls_aliasname[], ref string ls_expression[]);//char ls_sql[]
...
for l_index=1 to l_length //1
	...
	if ls_temp=',' then 
		...
		if ls_types='FIELDS' then
			ls_text+=ls_singleword
			i++
			ls_words[i]=trim(ls_text)
			goto nex
		end if
	end if
	
	if trim(ls_temp)='' then //2
		//if ls_singleword='' then continue;
		if ls_types='' then //3
...
for l_index=l_index to l_count
	...
	if ls_fieldstr='' then 
		ls_fieldstr+=ls_alias+'='+ls_dbname
	else
		ls_fieldstr+=','+ls_alias+'='+ls_dbname
	end if
next
...
for i=1 to li_NumOutputs
	CHOOSE CASE SQLDA.OutParmType[i]
		...
		CASE TypeInteger!,TypeLong!,TypeUInt!,TypeUInt!,TypeULong! //,TypeByte!
			...
			a_parm[i]=Intvar
		CASE TypeDecimal!,TypeReal!,TypeDouble! //TypeLongLong!,
...
this.ids=create ids
this.ids_empty=create ids_empty
...
event dberror;is_dberror=string(sqldbcode)+' '+sqlerrtext+' '+sqlsyntax
end event
...
type ids_empty from datastore within u_derek_v2_dd descriptor "pb_nvo" = "true" 
end type
...
on ids_empty.destroy
TriggerEvent( this, "destructor" )
call super::destroy
end on

event constructor;create('release 9;table()')
end event
